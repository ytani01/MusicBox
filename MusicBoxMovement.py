...
class MusicBoxMovementBase(threading.Thread):
    ...
    def end(self):
        ...
        if self.active:
            self._sound_q.put(self.MOVEMENT_END)
            self.join()
...
class MusicBoxMovement(MusicBoxMovementBase):
    ...
    def __init__(self,
        ...
        self._mtr = MusicBoxRotationMotor(pin1, pin2, pin3, pin4,
                                          debug=False)
        if 0 < rotation_speed <= 10:
            self._mtr.set_speed(rotation_speed)
...
class SampleApp:
    ...
    def play_file(self, paper_tape_file):
        ...
        self._movement.play_music(music_data)
        """
        for i, d in enumerate(music_data):
            self._log.info('%4d: %s', i, d)
            self._movement.play_and_sleep(d['ch'], d['delay'])
        """
